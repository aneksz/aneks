...
# Paths
# ------------------------
THEME_DIR="$HOME/.config/themes/one-dark"
KITTY_CONF="$HOME/.config/kitty/kitty.conf"
KITTY_COLORS="$HOME/.config/kitty/colors.conf"
GTK4_THEME_DIR="$HOME/.local/share/themes/Orchis-Green-Dark/gtk-4.0"
...
# ------------------------
if [ -f "$THEME_DIR/kitty/onedark.colors.conf" ]; then
    cp "$THEME_DIR/kitty/onedark.colors.conf" "$KITTY_COLORS"

    # Update all running kitty windows
    for SOCKET in ~/.config/kitty/kitty.sock-*; do
        if [ -S "$SOCKET" ]; then
            export KITTY_LISTEN_ON="unix:$SOCKET"
            kitty @ set-colors --all --config "$THEME_DIR/kitty/onedark.colors.conf"
        fi
    done
fi
...
gsettings set org.gnome.desktop.interface icon-theme "Zafiro-Nord-Dark-Black"
...
swww img ~/.config/themes/one-dark/wallpapers/one-dark.png --outputs DP-1 --transition-type fade
swww img ~/.config/themes/one-dark/wallpapers/one-dark2.png --outputs DP-3 --transition-type fade

# ------------------------
#  Rofi Theme
# ------------------------

cp ~/.config/themes/one-dark/colors.rasi ~/.config/rofi/launchers/type-2/shared
...
if [ -f "$VSCODE_SETTINGS" ]; then
    sed -i 's/"workbench.colorTheme":[[:space:]]*"[^"]*"/"workbench.colorTheme": "One Dark"/' "$VSCODE_SETTINGS"
fi
...
# Save current theme
# ------------------------
echo "onedark" > "$HOME/.config/.current_theme"
...
cp "$HOME/.config/themes/one-dark/onedark.conf" "$HOME/.config/hypr/colors.conf"

ln -sf "$HOME/.config/hypr/background/one-dark.png" \
...
cp "$HOME/.config/themes/one-dark/onedark.conf" \
