...
# Wallpaper
# ------------------------
awww img "$THEME_DIR/wallpapers/forest1.png" --outputs DP-1 --transition-type grow
awww img "$THEME_DIR/wallpapers/forest2.png" --outputs DP-3 --transition-type grow
...
# Hyprlock
# ------------------------
ln -sf "$HYPR_DIR/background/forest1.png" "$HYPR_DIR/background/current.png"
...
cat << 'EOF' > ~/.config/.theme_terminal
export EZA_FLAGS="--icons --color=auto"
export FASTFETCH_CONFIG="-c ~/.local/share/fastfetch/13.jsonc"
EOF

source ~/.config/.theme_terminal
