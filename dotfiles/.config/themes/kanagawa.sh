...
if [ -f "$THEME_DIR/kitty/kanagawa.colors.conf" ]; then
    ...
    for SOCKET in ~/.config/kitty/kitty.sock-*; do
        if [ -S "$SOCKET" ]; then
            ...
            kitty @ set-colors --all --config "$THEME_DIR/kitty/kanagawa.colors.conf"
        fi
    done
fi
...
# Wallpaper
# ------------------------
awww img "$THEME_DIR/wallpapers/kanagawa1.png" --outputs DP-1 --transition-type any
awww img "$THEME_DIR/wallpapers/kanagawa2.png" --outputs DP-3 --transition-type any
