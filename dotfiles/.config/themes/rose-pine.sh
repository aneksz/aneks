...
# Paths
# ------------------------
THEME_DIR="$HOME/.config/themes/rose-pine"
KITTY_CONF="$HOME/.config/kitty/kitty.conf"
KITTY_COLORS="$HOME/.config/kitty/colors.conf"
GTK4_THEME_DIR="$HOME/.local/share/themes/Rosepine-BL-MB-Dark/gtk-4.0"
...
# ------------------------
if [ -f "$THEME_DIR/kitty/rose.colors.conf" ]; then
    cp "$THEME_DIR/kitty/rose.colors.conf" "$KITTY_COLORS"

    # Update all running kitty windows
    for SOCKET in ~/.config/kitty/kitty.sock-*; do
        if [ -S "$SOCKET" ]; then
            export KITTY_LISTEN_ON="unix:$SOCKET"
            kitty @ set-colors --all --config "$THEME_DIR/kitty/rose.colors.conf"
        fi
    done
fi
...
swww img ~/.config/themes/rose-pine/wallpapers/rose3.png --outputs DP-1 --transition-type fade
swww img ~/.config/themes/rose-pine/wallpapers/rose4.png --outputs DP-3 --transition-type fade

# ------------------------
#  Rofi Theme
# ------------------------

cp ~/.config/themes/rose-pine/colors.rasi ~/.config/rofi/launchers/type-2/shared
...
if [ -f "$VSCODE_SETTINGS" ]; then
    sed -i 's/"workbench.colorTheme":[[:space:]]*"[^"]*"/"workbench.colorTheme": "Rosé Pine"/' "$VSCODE_SETTINGS"
fi
...
spicetify config current_theme Rose-Pine
spicetify apply -n
...
cp "$HOME/.config/themes/rose-pine/rose.conf" "$HOME/.config/hypr/colors.conf"
y
ln -sf "$HOME/.config/hypr/background/rose3.png" \
...
cp "$HOME/.config/themes/rose-pine/rose.conf" \
